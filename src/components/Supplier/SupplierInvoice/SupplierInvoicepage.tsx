...
const SupplierInvoicePage= () => {
    ...
    || { items: [], pagination: { currentPage: 0, totalPages: 0, pageSize: 0, totalCount: 0 } }

    const totalAmount = supplierProducts.items.reduce((acc, item) => acc + ((item.cost || 1) * item.quantity) , 0)
    const supplierName = supplierProducts.items[0]?.supplier?.name || 'Supplier Name'
  return (
    <Container>
